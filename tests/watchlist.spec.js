...
// 1. Mocked database to protect production and live data
jest.mock('../db/connect', () => {
  const store = [
    {
      _id: '507f1f77bcf86cd799439011',
...
// 2. Mock authentication middleware to automatically let test requests pass through
jest.mock('../middleware/isAuthenticated', () => ({
  ensureAuthenticated: (req, res, next) => {
    // Emulate a successfully decoded token payload for the test environment
...
describe('Watchlist Collection Unit Test', () => {
  ...
  describe('GET /watchlist/user/:userId', () => {
    it('should return 200 and watchlist items for a valid user', async () => {
      ...
      // Fixed: changed to match array response matching the collection find block layout
      expect(res.body[0]).toHaveProperty('userId', 'user123');
    });
